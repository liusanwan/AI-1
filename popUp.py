...
url = "https://sqpx.91huayi.com/AccountCenter/register.aspx"
# url = "https://www.ebidding.com/ebidding/SysRegAction.do?&method=register&no_sitemesh&sid=zmzb"
...
color_list_disc = {'红色':"rgb(224 22 22 / 39%)",'灰色':"rgb(229 229 243 / 45%)",'粉色':"rgb(226 110 236 / 33%)"
    ...
    ,'蓝色':"rgb(15 224 191 / 53%)",'紫色':"rgb(157 70 228 / 26%)",'深灰色':"rgb(8 8 8 / 26%)",'深灰色':"rgb(8 8 8 / 26%)"
    ,'深灰色':"rgb(8 8 8 / 26%)",'深灰色':"rgb(8 8 8 / 26%)",'深灰色':"rgb(8 8 8 / 26%)",'深灰色':"rgb(8 8 8 / 26%)"}
# yang
...
def button_style(i,temp_elem,inputValue):
    if temp_elem == "space":
        ...
        element = temp_elem
        global textinsertBtn
        textinsertBtn = inputValue +"\n"
        if i =="":
            textBox.insert('end', textinsertBtn )
        xpath = element['B_xPath']
        b = driver.find_element_by_xpath(xpath)
    ...
    if checkValue==1:
        ...
        for element in input_list:
            ...
            if element['B_text'] =="":
                ...
                js_code = 'arguments[0].style.border = "2px green solid";arguments[0].style.color = "#33333352";arguments[0].type="text";'
                try:
                    b.clear()
                    b.send_keys(text)
                    driver.execute_script(js_code, b)
                    ...
                    pass
val = tkinter.IntVar()
chkbt = tkinter.Checkbutton(win, text='是否显示lable文字', bd=2, relief='raised', underline=0, command=test_cb, variable=val)
chkbt.place(x=10,y=290)
try:
    win.mainloop()
except:
    pass